function footer() {
    document.getElementById("footer_js").innerHTML=`
    <footer id="footer">
      <div class="main">
        <div class="main__top">
          <div class="main__top__search">
            <input
              type="text"
              autocomplete="off"
              placeholder="Search Time..."
            />
          </div>
          <div class="main__top__search1">
            <span class="search">Search <i class="fas fa-arrow-right"></i> </span>
          </div>
        </div>

        <div class="main__mid">
          <!-- col 1 -->
          <div class="col1">
            <a href="../html/World.html" class="heading">World</a>
            <ul>
              <li><a href="../html/Africa.html">Africa</a></li>
              <li><a href="../html/Americas.html">Americas</a></li>
              <li><a href="../html/Asia.html">Asia</a></li>
              <li><a href="../html/Australia.html">Australia</a></li>
              <li><a href="../html/China.html">China</a></li>
              <li><a href="../html/Europe.html">Europe</a></li>
              <li><a href="../html/India.html">India</a></li>
              <li><a href="../html/MiddleEast.html">Middle East</a></li>
              <li><a href="../html/UnitedKingdom.html">United Kingdom</a></li>
            </ul>
          </div>

          <!-- col 2  -->
          <div class="col2">
            <a href="#" class="heading">US Politics</a>
            <ul>
              <li><a href="#">The Biden Presidency</a></li>
              <li><a href="#">Facts First</a></li>
              <li><a href="#">US Elections</a></li>
            </ul>
          </div>
          <!-- col 3  -->
          <div class="col3">
            <a href="#" class="heading">Business</a>
            <ul>
              <li><a href="#">Markets</a></li>
              <li><a href="#">Tech</a></li>
              <li><a href="#">Media</a></li>
              <li><a href="#">Success</a></li>
              <li><a href="#">Perspectives</a></li>
              <li><a href="#">Videos</a></li>
            </ul>
          </div>
          <!-- col 4 -->
          <div class="col4">
            <a href="#" class="heading">Health</a>
            <ul>
              <li><a href="#">Life, But Better</a></li>
              <li><a href="#">Fitness</a></li>
              <li><a href="#">Food</a></li>
              <li><a href="#">Sleep</a></li>
              <li><a href="#">Mindfulness</a></li>
              <li><a href="#">Relationships</a></li>
            </ul>
          </div>

          <!-- col 5 -->
          <div class="col5">
            <a href="#" class="heading">Entertainment</a>
            <ul>
              <li><a href="#">Stars</a></li>
              <li><a href="#">Screen</a></li>
              <li><a href="#">Binge</a></li>
              <li><a href="#">Culture</a></li>
              <li><a href="#">Media</a></li>
            </ul>
          </div>
          <!-- col 6 -->
          <div class="col6">
            <a href="#" class="heading">Tech</a>
            <ul>
              <li><a href="#">Innovate</a></li>
              <li><a href="#">Gadget</a></li>
              <li><a href="#">Foreseeable Future</a></li>
              <li><a href="#">Mission: Ahead</a></li>
              <li><a href="#">Upstarts</a></li>
              <li><a href="#">Work Transformed</a></li>
              <li><a href="#">Innovative Cities</a></li>
            </ul>
          </div>
          <!-- col 7  -->
          <div class="col7">
            <a href="#" class="heading">Style</a>
            <ul>
              <li><a href="#">Arts</a></li>
              <li><a href="#">Design</a></li>
              <li><a href="#">Fashion</a></li>
              <li><a href="#">Architecture</a></li>
              <li><a href="#">Luxury</a></li>
              <li><a href="#">Beauty</a></li>
              <li><a href="#">Video</a></li>
            </ul>
          </div>

          <!-- col 8  -->
          <div class="col8">
            <a href="#" class="heading">Travel</a>
            <ul>
              <li><a href="#">Destinations</a></li>
              <li><a href="#">Food and Drink</a></li>
              <li><a href="#">Stay</a></li>
              <li><a href="#">News</a></li>
              <li><a href="#">Videos</a></li>
            </ul>
          </div>
          <!-- col 9 -->
          <div class="col9">
            <a href="#" class="heading">Sports</a>
            <ul>
              <li><a href="#">Football</a></li>
              <li><a href="#">Tennis</a></li>
              <li><a href="#">Golf</a></li>
              <li><a href="#">Tokyo 2020</a></li>
              <li><a href="#">US Sports</a></li>
              <li><a href="#">Climbing</a></li>
              <li><a href="#">Motorsport</a></li>
              <li><a href="#">Formula E</a></li>
              <li><a href="#">Esports</a></li>
            </ul>
          </div>

          <!-- col 10 -->
          <div class="col10">
            <a href="#" class="heading">Videos</a>
            <ul>
              <li><a href="#">Live TV</a></li>
              <li><a href="#">Digital Studios</a></li>
              <li><a href="#">TIME Films</a></li>
              <li><a href="#">HLN</a></li>
              <li><a href="#">TV Schedule</a></li>
              <li><a href="#">TV Shows A-Z</a></li>
              <li><a href="#">TIMEVR</a></li>
            </ul>
          </div>
          <!-- col 11 -->
          <div class="col11">
            <a href="#" class="heading">Features</a>
            <ul>
              <li><a href="#">As Equals</a></li>
              <li><a href="#">Call to Earth</a></li>
              <li><a href="#">Freedom Project</a></li>
              <li><a href="#">Impact Your World</a></li>
              <li><a href="#">Inside Africa</a></li>
              <li><a href="#">2 Degrees</a></li>
              <li><a href="#">TIME Heroes</a></li>
              <li><a href="#">All Features</a></li>
            </ul>
          </div>
          <!-- col 12  -->
          <div class="col12">
            <a href="#" class="heading">Weather</a>
            <ul>
              <li><a href="#">Climate</a></li>
              <li><a href="#">Storm Tracker</a></li>
              <li><a href="#">Wildfire Tracker</a></li>
              <li><a href="#">Video</a></li>
            </ul>
          </div>
          <!-- col 13 -->
          <div class="col13">
            <a href="#" class="heading">More</a>
            <ul>
              <li><a href="#">Photos</a></li>
              <li><a href="#">Longform</a></li>
              <li><a href="#">Investigations</a></li>
              <li><a href="../html/info.html">Information About Members</a></li>
              <li><a href="../html/term.html">Term of Uses</a></li>
              <li><a href="../html/privacy.html">News & Sports Privacy Policy</a></li>
            </ul>
          </div>
        </div>
        <div class="main__bottom">
          <div class="facebook">
            <a href="https://www.facebook.com/vu.datnuoc/" target="_blank"
              ><i class="fab fa-facebook-square"></i
            ></a>
          </div>
          <div class="twitter">
            <a href="https://twitter.com/VNguyn86791271" target="_blank"
              ><i class="fab fa-twitter"></i
            ></a>
          </div>
          <div class="instagram">
            <a href="#"> <i class="fab fa-instagram"></i></a>
          </div>
          <div class="follow_us"><a href="../html/SiteMap.html">SiteMap</a></div>
        </div>
      </div>
      <div>
      <h5 class="info" style="border-top: 1px solid white;">&copy; 2022 Cable News Network.A Warner Bros. Discovery Company.All Rights Reserved.
        CNN Sans ™ & © 2016 Cable News Network.</h5>
        <p>
          <a href="./html/SiteMap.html">SITE MAP</a> | <a href="./html/info.html">INFORMATION</a>
        </p>
     </div>
    </footer>`
}
footer()